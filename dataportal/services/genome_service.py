...
class GenomeService:
    ...
    async def get_type_strains(self) -> List[TypeStrainSchema]:
        try:
            ...
            return [TypeStrainSchema.model_validate(strain.__dict__) for strain in strains]
        except Exception as e:
            logger.error(f"Error fetching type strains: {e}")
            raise HttpError(500, "Internal Server Error")

    async def search_genomes_by_string(self, query: str, page: int = 1, per_page: int = 10) -> GenomePaginationSchema:
        try:
            strains, total_results = await self._fetch_paginated_strains(Q(isolate_name__icontains=query), page,
                                                                         per_page)
    ...
    async def get_genomes_by_species(self, species_id: int, page: int = 1,
                                     per_page: int = 10) -> GenomePaginationSchema:
        try:
            strains, total_results = await self._fetch_paginated_strains(Q(species_id=species_id), page, per_page)
            return await self._create_pagination_schema(strains, total_results, page, per_page)
        except Exception as e:
            logger.error(f"Error fetching genomes by species: {e}")
            raise HttpError(500, "Internal Server Error")

    async def search_genomes_by_species_and_string(self, species_id: int, query: str, page: int = 1,
                                                   per_page: int = 10) -> GenomePaginationSchema:
        try:
            filter_criteria = Q(species_id=species_id, isolate_name__icontains=query)
            strains, total_results = await self._fetch_paginated_strains(filter_criteria, page, per_page)
            return await self._create_pagination_schema(strains, total_results, page, per_page)
        except Exception as e:
    ...
    async def get_genomes(self, page: int = 1, per_page: int = 10) -> GenomePaginationSchema:
        try:
            # Fetch paginated strains
            strains, total_results = await self._fetch_paginated_strains(Q(), page, per_page)
            return await self._create_pagination_schema(strains, total_results, page, per_page)
        except Exception as e:
            logger.error(f"Error fetching genomes: {e}")
            raise HttpError(500, "Internal Server Error")

    async def search_genomes(self, query: str, page: int = 1, per_page: int = 10) -> GenomePaginationSchema:
        try:
    ...
    async def get_genome_by_id(self, genome_id: int):
        try:
            strain = await sync_to_async(lambda: Strain.objects.select_related('species').get(id=genome_id))()
            return SearchGenomeSchema.model_validate({
                ...
                "fasta_url": f"{settings.ASSEMBLY_FTP_PATH}{strain.assembly_name}/{strain.fasta_file}" if strain.fasta_file else None,
                "gff_url": f"{settings.GFF_FTP_PATH.format(strain.isolate_name)}{strain.gff_file}" if strain.gff_file else None,
                "contigs": await sync_to_async(lambda: [
    ...
    # Helper Methods
    async def _fetch_paginated_strains(self, filter_criteria: Q, page: int, per_page: int):
        start = (page - 1) * per_page
        strains = await sync_to_async(lambda: list(
            Strain.objects.select_related('species').filter(filter_criteria).order_by('isolate_name')[
            start:start + per_page]
        ))()
        total_results = await sync_to_async(Strain.objects.filter(filter_criteria).count)()
