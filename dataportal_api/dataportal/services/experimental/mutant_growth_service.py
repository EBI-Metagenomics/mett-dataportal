...
class MutantGrowthService(BaseService[MutantGrowthWithGeneSchema, str]):
    ...
    def __init__(self):
        super().__init__(INDEX_FEATURE_EXPERIMENTS)
    ...
    def _fetch_mutant_growth_by_identifier(self, identifier: str) -> MutantGrowthWithGeneSchema:
        ...
        if not response.hits:
            raise GeneNotFoundError(
                f"Gene with identifier '{identifier}' not found or has no mutant growth data"
            )
    ...
    def _fetch_mutant_growth_by_identifiers(
        ...
        if any(
            [
                ...
                min_doubling_time is not None,
                max_doubling_time is not None,
                exclude_double_picked is not None,
            ]
        ):
            nested_conditions = []
            ...
            if experimental_condition:
                nested_conditions.append(
                    {"term": {"mutant_growth.experimental_condition": experimental_condition}}
                )
            if min_doubling_time is not None:
                nested_conditions.append(
                    {"range": {"mutant_growth.doubling_time": {"gte": min_doubling_time}}}
                )
            if max_doubling_time is not None:
                nested_conditions.append(
                    {"range": {"mutant_growth.doubling_time": {"lte": max_doubling_time}}}
                )
            ...
            if nested_conditions:
                s = s.filter(
                    "nested", path="mutant_growth", query={"bool": {"must": nested_conditions}}
                )
                ...
                if schema.mutant_growth and len(schema.mutant_growth) > 0:
                    ...
                    for entry in schema.mutant_growth:
                        ...
                        if media and entry.media != media:
                            continue
                        if min_doubling_time is not None and (
                            entry.doubling_time is None or entry.doubling_time < min_doubling_time
                        ):
                            continue
                        if max_doubling_time is not None and (
                            entry.doubling_time is None or entry.doubling_time > max_doubling_time
                        ):
                            continue
                    ...
                    logger.warning(
                        f"Gene {schema.locus_tag} has has_mutant_growth=True but empty mutant_growth array"
                    )
            except Exception as e:
    ...
    def _convert_hit_to_mutant_growth_schema(self, hit) -> MutantGrowthWithGeneSchema:
        ...
        return MutantGrowthWithGeneSchema(**gene_data, mutant_growth=mutant_growth_data)
