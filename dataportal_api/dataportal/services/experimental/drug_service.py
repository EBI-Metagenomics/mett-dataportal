...
class DrugService(BaseService[StrainDrugMICResponseSchema, Dict[str, Any]]):
    ...
    async def get_all(self, **kwargs) -> List[StrainDrugMICResponseSchema]:
        ...
        raise NotImplementedError(
            "get_all not implemented for drug service - use search methods instead"
        )
    ...
    async def search(self, query: Dict[str, Any]) -> List[StrainDrugMICResponseSchema]:
        ...
        raise NotImplementedError(
            "search not implemented for drug service - use specific search methods instead"
        )
    ...
    async def get_strain_drug_metabolism(
        self, isolate_name: str
    ) -> Optional[StrainDrugMetabolismResponseSchema]:
    ...
    async def get_strain_drug_data(
        self, isolate_name: str
    ) -> Optional[StrainDrugDataResponseSchema]:
    ...
    @log_execution_time
    async def get_strain_drug_mic_paginated(
        self, isolate_name: str, page: int = 1, per_page: int = 20
    ) -> Optional[PaginatedStrainDrugMICResponseSchema]:
    ...
    @log_execution_time
    async def get_strain_drug_metabolism_paginated(
        self, isolate_name: str, page: int = 1, per_page: int = 20
    ) -> Optional[PaginatedStrainDrugMetabolismResponseSchema]:
        ...
        try:
            ...
            logger.error(
                f"Error getting paginated strain drug metabolism data for {isolate_name}: {e}"
            )
            raise ServiceError(f"Failed to retrieve paginated drug metabolism data: {str(e)}")

    @log_execution_time
    async def get_strain_drug_data_paginated(
        self, isolate_name: str, page: int = 1, per_page: int = 20
    ) -> Optional[PaginatedStrainDrugDataResponseSchema]:
        ...
        try:
            ...
            paginated_metab = [
                item["data"] for item in paginated_combined if item["type"] == "metabolism"
            ]
        ...
        try:
            ...
            if query.drug_name:
                ...
                drug_query = Q(
                    ...
                    should=[
                        ...
                        Q(
                            "fuzzy",
                            drug_mic__drug_name__keyword={
                                "value": query.drug_name.lower(),
                                "fuzziness": "AUTO",
                            },
                        ),
            ...
            if query.experimental_condition:
                nested_queries.append(
                    Q(
                        "term",
                        drug_mic__experimental_condition_name__keyword=query.experimental_condition,
                    )
                )
            ...
            if nested_queries:
                main_queries.append(
                    Q("nested", path="drug_mic", query=Q("bool", must=nested_queries))
                )
            elif any(
                [
                    query.drug_name,
                    ...
                    query.min_mic_value is not None,
                    query.max_mic_value is not None,
                ]
            ):
            ...
            if query.query:
                ...
                top_level_query = Q(
                    "multi_match", query=query.query, fields=["isolate_name", "species_acronym"]
                )
            ...
            if query.sort_by and query.sort_by in [
                "isolate_name",
                "species_acronym",
                ...
                sort_field = (
                    f"{query.sort_by}.keyword" if query.sort_by == "isolate_name" else query.sort_by
                )
            ...
            for hit in response.hits:
                ...
                for mic_data in source.get("drug_mic", []):
                    ...
                    if (
                        query.experimental_condition
                        and mic_data.get("experimental_condition_name")
                        != query.experimental_condition
                    ):
                        continue
                    if (
                        query.min_mic_value is not None
                        and mic_data.get("mic_value", 0) < query.min_mic_value
                    ):
                        continue
                    if (
                        query.max_mic_value is not None
                        and mic_data.get("mic_value", float("inf")) > query.max_mic_value
                    ):
                        continue
    ...
    @log_execution_time
    async def search_drug_metabolism(
        self, query: DrugMetabolismSearchQuerySchema
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if query.drug_name:
                ...
                drug_query = Q(
                    ...
                    should=[
                        ...
                        Q(
                            "fuzzy",
                            drug_metabolism__drug_name__keyword={
                                "value": query.drug_name.lower(),
                                "fuzziness": "AUTO",
                            },
                        ),
                        Q(
                            "wildcard",
                            drug_metabolism__drug_name__keyword=f"*{query.drug_name.lower()}*",
                        ),
                    ],
                )
                nested_queries.append(drug_query)
            ...
            if query.drug_class:
                nested_queries.append(
                    Q("term", drug_metabolism__drug_class__keyword=query.drug_class)
                )
            ...
            if query.metabolizer_classification:
                nested_queries.append(
                    Q(
                        "term",
                        drug_metabolism__metabolizer_classification__keyword=query.metabolizer_classification,
                    )
                )
            ...
            if query.experimental_condition:
                nested_queries.append(
                    Q(
                        "term",
                        drug_metabolism__experimental_condition_name__keyword=query.experimental_condition,
                    )
                )

            if query.is_significant is not None:
                nested_queries.append(
                    Q("term", drug_metabolism__is_significant=query.is_significant)
                )
            ...
            if query.min_degr_percent is not None:
                nested_queries.append(
                    Q("range", drug_metabolism__degr_percent={"gte": query.min_degr_percent})
                )

            # Add nested query if we have any drug_metabolism filters
            if nested_queries:
                main_queries.append(
                    Q("nested", path="drug_metabolism", query=Q("bool", must=nested_queries))
                )
            ...
            for hit in response.hits:
                ...
                for metab_data in source.get("drug_metabolism", []):
                    ...
                    if query.drug_class and metab_data.get("drug_class") != query.drug_class:
                        continue
                    if (
                        query.metabolizer_classification
                        and metab_data.get("metabolizer_classification")
                        != query.metabolizer_classification
                    ):
                        continue
                    if (
                        query.experimental_condition
                        and metab_data.get("experimental_condition_name")
                        != query.experimental_condition
                    ):
                        continue
                    if (
                        query.is_significant is not None
                        and metab_data.get("is_significant") != query.is_significant
                    ):
                        continue
                    if query.min_fdr is not None and metab_data.get("fdr", 1.0) > query.min_fdr:
                        continue
                    if (
                        query.min_degr_percent is not None
                        and metab_data.get("degr_percent", 0) < query.min_degr_percent
                    ):
                        continue
                    ...
                    results.append(
                        DrugMetabolismSearchResultSchema(
                            ...
                            experimental_condition_name=metab_data.get(
    ...
    async def get_drug_mic_by_drug(
        self, drug_name: str, species_acronym: Optional[str] = None
    ) -> List[DrugMICSearchResultSchema]:
        ...
        try:
            ...
            logger.info(
                f"Elasticsearch query for drug MIC by drug '{drug_name}' with species '{species_acronym}': {search.to_dict()}"
            )

            # Scan all matching documents to avoid default size limits
            response_hits = await self._scan_search(search)
                    ...
                    if drug_name_lower in actual_drug_name or actual_drug_name in drug_name_lower:
                        results.append(
                            DrugMICSearchResultSchema(
                                ...
                                unit=mic_data.get("unit"),
                                relation=mic_data.get("relation"),
                                experimental_condition_name=mic_data.get(
                                    "experimental_condition_name"
                                ),
                            )
                        )
    ...
    async def get_drug_metabolism_by_drug(
        self, drug_name: str, species_acronym: Optional[str] = None
    ) -> List[DrugMetabolismSearchResultSchema]:
                    ...
                    if drug_name_lower in actual_drug_name or actual_drug_name in drug_name_lower:
                        results.append(
                            DrugMetabolismSearchResultSchema(
                                ...
                                pval=metab_data.get("pval"),
                                fdr=metab_data.get("fdr"),
                                metabolizer_classification=metab_data.get(
    ...
    @log_execution_time
    async def get_drug_suggestions(
        self, query: DrugAutocompleteQuerySchema
    ) -> List[DrugSuggestionSchema]:
        ...
        try:
            ...
            drug_query = Q(
                "wildcard", **{f"{nested_path}__drug_name__keyword": f"*{query.query.lower()}*"}
            )
            ...
            for drug_name, count in sorted(drug_counts.items(), key=lambda x: x[1], reverse=True):
                suggestions.append(
                    DrugSuggestionSchema(
                        drug_name=drug_name, drug_class=drug_classes[drug_name], count=count
                    )
                )
