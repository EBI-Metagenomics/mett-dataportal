...
class ReactionsService(BaseService[ReactionsWithGeneSchema, str]):
    ...
    def _fetch_reactions_by_identifier(self, identifier: str) -> ReactionsWithGeneSchema:
        ...
        if not response.hits:
            raise GeneNotFoundError(
                f"Gene with identifier '{identifier}' not found or has no reactions data"
            )
    ...
    def _fetch_reactions_by_identifiers(
        ...
        s = Search(index=self.index_name).filter("term", has_reactions=True)  # Fast boolean filter
        ...
        if any([reaction_id, substrate, product]):
            ...
            if nested_conditions:
                s = s.filter(
                    "nested", path="reaction_details", query={"bool": {"must": nested_conditions}}
                )
                ...
                if schema.reaction_details and len(schema.reaction_details) > 0:
                    ...
                    for entry in schema.reaction_details:
                        ...
                        if reaction_id and entry.reaction != reaction_id:
                            continue
                        if substrate and (
                            not entry.substrates or substrate not in entry.substrates
                        ):
                            continue
                    ...
                    logger.warning(
                        f"Gene {schema.locus_tag} has has_reactions=True but empty reaction_details array"
                    )
            except Exception as e:
    ...
    def _convert_hit_to_reactions_schema(self, hit) -> ReactionsWithGeneSchema:
        ...
        return ReactionsWithGeneSchema(
            **gene_data, reactions=reactions_list, reaction_details=reaction_details_list
        )
