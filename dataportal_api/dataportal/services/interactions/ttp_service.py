...
class TTPService:
        ...
        if query_params.get("min_ttp_score") is not None:
            query.must.append(
                Q(
                    ...
                    path="protein_compound",
                    query=Q(
                        "range", protein_compound__ttp_score={"gte": query_params["min_ttp_score"]}
                    ),
                )
            )
        ...
        if query_params.get("max_ttp_score") is not None:
            query.must.append(
                Q(
                    ...
                    path="protein_compound",
                    query=Q(
                        "range", protein_compound__ttp_score={"lte": query_params["max_ttp_score"]}
                    ),
                )
            )
    ...
    async def search_interactions(
        self, query_schema: TTPInteractionQuerySchema
    ) -> TTPInteractionResponseSchema:
        ...
        try:
            ...
            if query_schema.sort_field:
                if query_schema.sort_field in ["ttp_score", "fdr", "compound", "hit_calling"]:
                    ...
                    search = search.sort(
                        {query_schema.sort_field: {"order": query_schema.sort_order}}
                    )
            ...
            logger.info(
                f"TTP Search Response: {response.hits.total} total hits, {len(response)} returned"
            )
            ...
            for hit in response:
                ...
                if search_query and matching_compound_found:
                    filtered = [
                        ci
                        for ci in compound_interactions_for_hit
                        if ci.compound and search_query in ci.compound.lower()
                    ]
            ...
            interactions = [
                TTPGeneInteractionSchema(**gene_data) for gene_data in gene_interactions.values()
            ]
    ...
    async def get_gene_interactions(
        self, query_schema: TTPGeneInteractionsQuerySchema
    ) -> TTPGeneInteractionSchema:
        ...
        try:
            ...
            logger.info(
                f"Debug - Total docs with locus_tag=PV_ATCC8482_00051: {locus_response.hits.total}"
            )
            ...
            if gene_interactions:
                ...
                logger.info(
                    f"TTP Gene Interactions Final Result: {len(gene_data['compounds'])} compounds returned"
                )
    ...
    async def get_compound_interactions(
        self, query_schema: TTPCompoundInteractionsQuerySchema
    ) -> List[TTPGeneInteractionSchema]:
        ...
        try:
            ...
            logger.info(
                f"TTP Compound Interactions Response: {response.hits.total} total hits, {len(response)} returned"
            )
    ...
    async def get_hit_analysis(
        self, query_schema: TTPHitAnalysisQuerySchema
    ) -> Tuple[List[TTPGeneInteractionSchema], TTPHitSummarySchema]:
    ...
    async def get_pool_analysis(
        self, query_schema: TTPPoolAnalysisQuerySchema
    ) -> TTPPoolSummarySchema:
        ...
        try:
            ...
            logger.info(
                f"TTP Pool Analysis Response: {response.hits.total} total hits, {len(response)} returned"
            )
            ...
            for hit in response:
                logger.debug(
                    f"Processing hit: {hit.locus_tag}, protein_compound count: {len(hit.protein_compound) if hasattr(hit, 'protein_compound') else 0}"
                )
                for pc in hit.protein_compound:
                    ...
                    logger.debug(
                        f"  Compound: {getattr(pc, 'compound', None)}, poolA: {pc_poolA}, poolB: {pc_poolB}"
                    )

                    # Check if this interaction matches the requested pool combination
                    if (
                        query_schema.poolA
                        and query_schema.poolB
                        and pc_poolA == query_schema.poolA
                        and pc_poolB == query_schema.poolB
                    ):
                        total_interactions += 1
                        compound = getattr(pc, "compound", None)
                        if compound:
                        ...
                        logger.debug(
                            f"    -> MATCH: Added interaction (total now: {total_interactions})"
                        )
                    elif (
                        query_schema.poolA
                        and not query_schema.poolB
                        and pc_poolA == query_schema.poolA
                    ):
                        # Only poolA specified
                        total_interactions += 1
                        ...
                        if compound:
                            ...
                            if getattr(pc, "hit_calling", False):
                                ...
                                compound_hits[compound] = compound_hits.get(compound, 0) + 1
                        logger.debug(
                            f"    -> MATCH (poolA only): Added interaction (total now: {total_interactions})"
                        )
                    elif (
                        query_schema.poolB
                        and not query_schema.poolA
                        and pc_poolB == query_schema.poolB
                    ):
                        # Only poolB specified
                        total_interactions += 1
                        ...
                        if compound:
                            ...
                            if getattr(pc, "hit_calling", False):
                                ...
                                compound_hits[compound] = compound_hits.get(compound, 0) + 1
                        logger.debug(
                            f"    -> MATCH (poolB only): Added interaction (total now: {total_interactions})"
                        )
                    else:
                        logger.debug(
                            f"    -> NO MATCH: poolA={pc_poolA}, poolB={pc_poolB} vs requested poolA={query_schema.poolA}, poolB={query_schema.poolB}"
                        )

            logger.info(
                f"Pool Analysis: Found {total_interactions} interactions, {total_hits} hits, {len(compounds)} unique compounds"
            )
            ...
            top_compounds = [
                {"compound": compound, "hit_count": count} for compound, count in top_compounds
            ]
        ...
        try:
            ...
            if interactions:
                writer = csv.DictWriter(
                    output, fieldnames=interactions[0].keys(), delimiter=delimiter
                )
                writer.writeheader()
