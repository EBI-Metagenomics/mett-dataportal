...
class OrthologSearchQuerySchema(BaseModel):
    ...
    locus_tag: Optional[str] = Field(
        None,
        description="Search for orthologs of this gene (locus tag)"
    )
    ...
    orthology_type: Optional[str] = Field(
        None,
        description="Filter by orthology type (1:1, many:1, 1:many, many:many)"
    )
    one_to_one_only: bool = Field(
        False,
        description="Return only one-to-one orthologs"
    )
...
class OrthologGeneInfoSchema(BaseModel):
    ...
    orthology_type: Optional[str] = Field(None, description="Type of orthology relationship (1:1, many:1, etc.)")
