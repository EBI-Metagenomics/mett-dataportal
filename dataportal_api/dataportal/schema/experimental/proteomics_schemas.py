...
class ProteomicsDataSchema(BaseModel):
    ...
    unique_intensity: Optional[float] = Field(
        None, description="Unique peptide intensity measurement"
    )
...
class ProteomicsWithGeneSchema(BaseModel):
    ...
    proteomics: Optional[List[ProteomicsDataSchema]] = Field(
        default_factory=list, description="Array of proteomics evidence entries for this gene"
    )
...
class ProteomicsSearchQuerySchema(BaseModel):
    ...
    min_coverage: Optional[float] = Field(
        None, description="Minimum coverage percentage filter", ge=0, le=100
    )
    min_unique_peptides: Optional[int] = Field(
        None, description="Minimum number of unique peptides filter", ge=0
    )
    has_evidence: Optional[bool] = Field(
        None, description="Filter by proteomics evidence flag (true/false)"
    )
