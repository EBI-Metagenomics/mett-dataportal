...
class EssentialityDataSchema(BaseModel):
    ...
    tas_in_locus: Optional[int] = Field(
        None, alias="TAs_in_locus", description="Number of transposon insertion sites in locus"
    )
    tas_hit: Optional[float] = Field(
        None, alias="TAs_hit", description="Fraction of transposon sites hit (0-1)"
    )
    essentiality_call: Optional[str] = Field(
        None,
        description="Essentiality call (essential, not_essential, essential_solid, essential_liquid, unclear)",
    )
...
class EssentialityWithGeneSchema(BaseModel):
    ...
    essentiality_data: Optional[List[EssentialityDataSchema]] = Field(
        default_factory=list, description="Array of essentiality data entries for this gene"
    )
...
class EssentialitySearchQuerySchema(BaseModel):
    ...
    experimental_condition: Optional[str] = Field(
        None, description="Filter by experimental condition/media"
    )
    min_tas_in_locus: Optional[int] = Field(
        None, description="Minimum number of transposon insertion sites", ge=0
    )
    min_tas_hit: Optional[float] = Field(
        None, description="Minimum fraction of transposon sites hit (0-1)", ge=0, le=1
    )
    element: Optional[str] = Field(
        None, description="Filter by element type (gene, intergenic, etc.)"
    )
