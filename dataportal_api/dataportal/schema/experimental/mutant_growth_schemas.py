...
class MutantGrowthDataSchema(BaseModel):
    ...
    isdoublepicked: Optional[bool] = Field(
        None, description="Whether mutant was picked twice (not truly independent replicates)"
    )
    brep: Optional[str] = Field(
        None, description="Biological replicate identifier (brep_1, brep_2, etc.)"
    )
    ...
    percent_from_start: Optional[float] = Field(
        None, description="Transposon insertion position in gene (0-1)"
    )
...
class MutantGrowthWithGeneSchema(BaseModel):
    ...
    feature_id: Optional[str] = Field(
        None, description="Unique feature identifier (ES document ID)"
    )
    ...
    ig_locus_tag_a: Optional[str] = Field(
        None, description="First flanking gene locus tag (IG records only)"
    )
    ig_locus_tag_b: Optional[str] = Field(
        None, description="Second flanking gene locus tag (IG records only)"
    )
    flanking_locus_tags: Optional[List[str]] = Field(
        None, description="Both flanking gene locus tags (IG records only)"
    )

    # Mutant growth data (array of entries)
    mutant_growth: Optional[List[MutantGrowthDataSchema]] = Field(
        default_factory=list, description="Array of mutant growth data entries for this gene"
    )
...
class MutantGrowthSearchQuerySchema(BaseModel):
    ...
    locus_tags: Optional[str] = Field(
        None, description="Comma-separated list of locus tags to search for"
    )
    uniprot_ids: Optional[str] = Field(
        None, description="Comma-separated list of UniProt IDs to search for"
    )
    media: Optional[str] = Field(None, description="Filter by growth media")
    experimental_condition: Optional[str] = Field(
        None, description="Filter by experimental condition"
    )
    min_doubling_time: Optional[float] = Field(
        None, description="Minimum doubling time in hours", ge=0
    )
    max_doubling_time: Optional[float] = Field(
        None, description="Maximum doubling time in hours", ge=0
    )
    exclude_double_picked: Optional[bool] = Field(
        None, description="Exclude double-picked mutants (isdoublepicked=True)"
    )
