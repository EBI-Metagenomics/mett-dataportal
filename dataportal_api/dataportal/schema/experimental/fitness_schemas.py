...
class FitnessDataSchema(BaseModel):
    ...
    number_of_barcodes: Optional[int] = Field(
        None, description="Number of barcodes (reliability indicator)"
    )
...
class FitnessWithGeneSchema(BaseModel):
    ...
    fitness: Optional[List[FitnessDataSchema]] = Field(
        default_factory=list, description="Array of fitness data entries for this gene"
    )
...
class FitnessSearchQuerySchema(BaseModel):
    ...
    # )
    contrast: Optional[str] = Field(None, description="Filter by contrast")
    min_lfc: Optional[float] = Field(None, description="Minimum log fold change (absolute value)")
    max_fdr: Optional[float] = Field(None, description="Maximum false discovery rate", ge=0, le=1)
    min_barcodes: Optional[int] = Field(None, description="Minimum number of barcodes", ge=0)
