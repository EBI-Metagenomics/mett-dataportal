...
class APITokenAdmin(admin.ModelAdmin):
    ...
    def token_info(self, obj):
        ...
        if not info:
            return format_html('<span style="color: red;">Invalid token format</span>')
        ...
        for key, value in info.items():
            ...
            html += (
                f'<tr><td style="padding: 5px; font-weight: bold;">{key}:</td>'
    ...
    def save_related(self, request, form, formsets, change):
        ...
        if should_regenerate:
            ...
            if self._token_was_generated:
                ...
                self.message_user(
                    request,
                    "⚠️ Token has been regenerated with updated roles. " "Old token is now invalid!",
                    level="warning",
                )
    ...
    def activate_tokens(self, request, queryset):
        ...
        self.message_user(request, f"{updated} token(s) activated successfully.")
    ...
    def deactivate_tokens(self, request, queryset):
        ...
        self.message_user(request, f"{updated} token(s) deactivated successfully.")
    ...
    def generate_new_token(self, request, queryset):
        ...
        for token_obj in queryset:
            # Generate new token
            token_obj.token = generate_jwt_token(name=token_obj.name, expiry_days=None)
            token_obj.save()
            count += 1

        self.message_user(
            request,
            f"{count} token(s) regenerated successfully. " "Note: Old tokens are now invalid!",
        )
