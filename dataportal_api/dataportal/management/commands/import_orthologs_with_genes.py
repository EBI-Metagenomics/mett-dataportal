...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if gff_parser and not no_gene_info:
            ...
            try:
                available_isolates = self._get_available_isolates_from_ftp(
                    ftp_server, ftp_directory
                )
                self.stdout.write(
                    f"Found {len(available_isolates)} available isolates from FTP server"
                )
                ...
                if available_isolates:
                    ...
                    if max_gff_files > 0 and len(available_isolates) > max_gff_files:
                        self.stdout.write(
                            f"Limiting to {max_gff_files} GFF files (use --max-gff-files 0 for all)"
                        )
                        ...
                        remaining_isolates = [
                            iso
                            for iso in sorted(available_isolates)
                            if iso not in priority_isolates
                        ]
                        selected_isolates = (
                            priority_isolates
                        ...
                        self.stdout.write(
                            f"Selected {len(available_isolates)} isolates for GFF loading"
                        )
                    ...
                    for isolate in sorted(available_isolates):
                        ...
                        species_name = species_map.get(
                            species_acronym, f"Unknown {species_acronym}"
                        )
                ...
                if all_isolates:
                    self.stdout.write(
                        f"Found {len(all_isolates)} unique isolates from files: {sorted(all_isolates)}"
                    )
        ...
        ortholog_flow = Orthologs(index_name=index_name, gff_parser=gff_parser)
