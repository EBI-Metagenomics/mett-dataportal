    ...
    try:
        ...
        if not result.proteomics or len(result.proteomics) == 0:
            return create_success_response(
                data=result, message=f"No proteomics evidence found for gene {locus_tag}"
            )

        return create_success_response(
            data=result, message=f"Proteomics evidence for gene {locus_tag} retrieved successfully"
        )
        ...
        raise_internal_server_error(f"Failed to fetch proteomics evidence for '{locus_tag}'")
...
async def search_proteomics(
    ...
    try:
        ...
        has_filters = (
            query.min_coverage is not None
            or query.min_unique_peptides is not None
            or query.has_evidence is not None
        )
        ...
        return create_success_response(
            data=results, message=f"Found {len(results)} genes with proteomics evidence"
        )
    except ServiceError as e:
        logger.error(f"Service error in proteomics search: {e}")
        raise_internal_server_error(f"Failed to search proteomics evidence: {str(e)}")
