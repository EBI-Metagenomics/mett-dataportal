    ...
    try:
        ...
        if not result.reactions or len(result.reactions) == 0:
            return create_success_response(
                data=result, message=f"No reactions data found for gene {locus_tag}"
            )

        return create_success_response(
            data=result, message=f"Reactions data for gene {locus_tag} retrieved successfully"
        )
...
async def search_reactions(
    ...
    try:
        ...
        has_filters = any([query.reaction_id, query.metabolite, query.substrate, query.product])
        ...
        return create_success_response(
            data=results, message=f"Found {len(results)} genes with reactions data"
        )
    except ServiceError as e:
