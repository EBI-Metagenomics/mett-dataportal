    ...
    try:
        ...
        if not result.mutant_growth or len(result.mutant_growth) == 0:
            return create_success_response(
                data=result, message=f"No mutant growth data found for gene {locus_tag}"
            )

        return create_success_response(
            data=result, message=f"Mutant growth data for gene {locus_tag} retrieved successfully"
        )
...
async def search_mutant_growth(
    ...
    try:
        ...
        return create_success_response(
            data=results, message=f"Found {len(results)} genes with mutant growth data"
        )
    except ServiceError as e:
