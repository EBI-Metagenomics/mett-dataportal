...
async def get_gene_orthologs(
    ...
    orthology_type: Optional[str] = Query(None, description="Filter by orthology type (1:1, many:1, etc.)"),
    ...
    try:
        ...
        return create_success_response(
            data=result,
            message=f"Found {result['total_count']} orthologs for gene {locus_tag}"
        )
    except ServiceError as e:
        logger.error(f"Service error: {e}")
        raise HttpError(500, f"Failed to get orthologs: {str(e)}")
...
async def get_ortholog_pair(
    ...
    try:
        ...
        if result is None:
            raise HttpError(404, f"No ortholog relationship found between {locus_tag_a} and {locus_tag_b}")
        ...
        logger.error(f"Service error: {e}")
        raise HttpError(500, f"Failed to get ortholog pair: {str(e)}")
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        raise HttpError(500, "Internal server error")
...
async def search_orthologs(
    ...
    try:
        ...
        return create_success_response(
            data=results,
            message=f"Found {results['total']} ortholog pairs"
        )
    except ServiceError as e:
        logger.error(f"Service error in ortholog search: {e}")
        raise HttpError(500, f"Failed to search orthologs: {str(e)}")
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        raise HttpError(500, "Internal server error")
...
async def get_orthology_types(request):
    ...
    try:
        ...
        return create_success_response(
            data={"orthology_types": types},
            message="Orthology types retrieved successfully"
        )
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        raise HttpError(500, "Internal server error")
