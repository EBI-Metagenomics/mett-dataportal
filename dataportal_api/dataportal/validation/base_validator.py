...
class ValidationService(BaseService[Dict, Dict]):
    ...
    def _validate_field_types(self, data: Dict[str, Any], field_types: Dict[str, type]) -> List[str]:
    ...
    def _validate_string_length(self, data: Dict[str, Any], field_lengths: Dict[str, int]) -> List[str]:
        ...
        for field, max_length in field_lengths.items():
            if field in data and isinstance(data[field], str) and len(data[field]) > max_length:
                errors.append(f"Field '{field}' must be no longer than {max_length} characters")
        return errors
    
    def _validate_numeric_range(self, data: Dict[str, Any], field_ranges: Dict[str, tuple]) -> List[str]:
