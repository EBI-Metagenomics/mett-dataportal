...
class Reactions(Flow):
    ...
    def run(
        self, gene_rx_path: str, met_rx_path: str, rx_gpr_path: str, chunksize: int = 50_000
    ) -> None:
...
""",
                ...
                self.add_feature_flag(actions, gene, "has_reactions")
        ...
        for chunk in chunks_from_table(rx_gpr_path, chunksize=100_000):
            ...
            for rec in chunk.to_dict(orient="records"):
                ...
                if not r:
                    continue
                d[str(r).strip()] = (
                    str(gpr).strip() if gpr not in (None, "", "nan", "NaN") else None
                )
