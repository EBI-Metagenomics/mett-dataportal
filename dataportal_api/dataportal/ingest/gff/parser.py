"""Shared GFF parser with in-memory caching for interaction ingest (PPI, orthologs, operons)."""
...
class GFFParser:
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt < retries - 1:
                    ...
                    delay = 2**attempt
                    logger.warning(
                        f"FTP connection failed, retrying in {delay}s... (attempt {attempt + 1}/{retries})"
                    )
                    time.sleep(delay)
                else:
    ...
    def _get_gff_file_for_isolate(self, isolate: str) -> Optional[str]:
        ...
        logger.debug(f"Getting GFF file for isolate: {isolate}")
        ...
        try:
            ...
            if gff_file:
                ...
                logger.warning(
                    f"No GFF file found for isolate {isolate} in directory {isolate_path}"
                )
    ...
    def _parse_gff_file(self, gff_file_path: str) -> Dict[str, GeneInfo]:
        ...
        logger.info(
            f"GFF parsing complete - Total lines: {line_count}, Genes found: {gene_count}, Skipped lines: {skipped_lines}"
        )
    ...
    def set_species_mapping(self, species_mapping: Dict[str, str]):
        ...
        logger.debug(f"Set species mapping: {species_mapping}")
    ...
    def preload_gff_files(self, species_list: List[str]) -> None:
        ...
        logger.debug(f"Pre-loading GFF files for {len(species_list)} species...")
        ...
        for i, species in enumerate(species_list, 1):
            ...
            try:
                logger.debug(f"Loading GFF file {i}/{len(species_list)}: {isolate}")
                self._load_isolate_gff_data(isolate)
                self._loaded_isolates.add(isolate)
                successful_loads += 1
                logger.debug(f"Loaded GFF for isolate: {isolate}")

                # Brief delay between downloads to avoid overwhelming the FTP server
    ...
    def get_gene_info_for_proteins(
        self, species: str, protein_a: str, protein_b: str
    ) -> Tuple[Optional[GeneInfo], Optional[GeneInfo]]:
        """Get gene information for both proteins in a PPI.
        Tries UniProt first (PPI data typically uses UniProt IDs); falls back to locus_tag.
        """
        gene_a = self._get_gene_info_by_protein_id(species, protein_a)
    ...
    def _get_gene_info_by_protein_id(self, species: str, protein_id: str) -> Optional[GeneInfo]:
        ...
        if not protein_id:
            return None
        # PPI typically uses UniProt IDs (e.g. A7V2E8); locus tags look like BU_ATCC8492_00001
        looks_like_locus = "_" in protein_id and (
            protein_id.startswith("BU_")
            or protein_id.startswith("PV_")
            or protein_id.startswith("IG:")
        )
        if looks_like_locus:
            gene = self.get_gene_info(species, protein_id)
            return gene or self.get_gene_info_by_uniprot(species, protein_id)
        gene = self.get_gene_info_by_uniprot(species, protein_id)
        return gene or self.get_gene_info(species, protein_id)
