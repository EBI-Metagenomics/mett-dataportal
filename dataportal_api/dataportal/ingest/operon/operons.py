...
class Operons(Flow):
    ...
    def __init__(self, index_name: str = "operon_index", gff_parser: Optional[GFFParser] = None):
        ...
        self._scratch: Dict[str, Dict[str, Counter]] = (
            {}
        )  # oid -> {"acronyms": Counter(), "isolates": Counter()}
    ...
    def run(self, path: str, chunksize: int = 5_000) -> None:
        ...
        for chunk in chunks_from_table(path, chunksize=chunksize):
            ...
            for rec in chunk.to_dict(orient="records"):
                ...
                ga_uniprot = _get(
                    rec, km, "gene_a_uniprot_id", "uniprot_a", "uniprot_id_a", "gene1_uniprot"
                )
                gb_uniprot = _get(
                    rec, km, "gene_b_uniprot_id", "uniprot_b", "uniprot_id_b", "gene2_uniprot"
                )

                ga_product = _get(
                    rec, km, "gene_a_product", "product_a", "gene1_product", "product_left"
                )
                gb_product = _get(
                    rec, km, "gene_b_product", "product_b", "gene2_product", "product_right"
                )
                ...
                if self.gff_parser:
                    ...
                    if ga and (
                        not doc["gene_a_name"]
                        or not doc["gene_a_product"]
                        or not doc["gene_a_uniprot_id"]
                    ):
                    ...
                    if gb and (
                        not doc["gene_b_name"]
                        or not doc["gene_b_product"]
                        or not doc["gene_b_uniprot_id"]
                    ):
        ...
        print(
            f"[operons-only] processed rows from {path}: {rows} (ignored w/o operon_id: {ignored})"
        )
