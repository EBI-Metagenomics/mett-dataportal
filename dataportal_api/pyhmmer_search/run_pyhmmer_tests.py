...
from pyhmmer_search.search.schemas import HitSchema
...
def test_single_domain_scenario():
    ...
    print(f"Expected: 1 domain, 1 significant domain")
...
def test_multiple_domains_mixed_significance():
    ...
    mock_hit, mock_domain1, mock_domain2, mock_job, mock_task_result = create_mock_objects()
    ...
    print(f"Domain 1 - Expected significant: True (score > 25.0, i_evalue < 0.01)")
    
    # Domain 2: Less significant
    print(f"Domain 2 - Score: {mock_domain2.score}, i_evalue: {mock_domain2.i_evalue}")
    print(f"Domain 2 - Expected significant: False (i_evalue > 0.01)")
    
    print(f"Expected: 2 domains, 1 significant domain")
    print()
...
def test_no_domains_scenario():
    ...
    print(f"Gene: {mock_hit.name.decode()}")
    print(f"Domains: {len(mock_hit.domains)}")
    print(f"Expected: 0 domains, 0 significant domains")
    print()
...
def test_fallback_threshold_calculation():
    ...
    print(f"Gene: {mock_hit.name.decode()}")
    print(f"hit.included attribute: Not available")
    print(f"Will use fallback calculation based on threshold type: {mock_job.threshold}")
    
    if mock_job.threshold == HmmerJob.ThresholdChoices.EVALUE:
        print(f"Fallback: i_evalue < {mock_job.incE} = {mock_domain.i_evalue < mock_job.incE}")
    else:
        print(f"Fallback: score > {mock_job.incT} = {mock_domain.score > mock_job.incT}")
    print()
...
def run_all_tests():
    ...
    print("cd dataportal_api && python -m pytest pyhmmer_search/search/test_tasks.py -v")
