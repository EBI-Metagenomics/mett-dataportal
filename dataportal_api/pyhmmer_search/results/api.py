...
def download_results(request, id: uuid.UUID, format: str):
    ...
    try:
        ...
        if format == "tab":
            ...
            content = generate_enhanced_aligned_fasta_content(
        ...
        if format in ["fasta", "aligned_fasta"]:
            ...
            try:
                ...
                content = (
                    generate_enhanced_fasta_content(result_data, db_path, job.input)
                    if format == "fasta"
                    else generate_enhanced_aligned_fasta_content(
                        result_data, db_path, job.input
                    )
                )
                if isinstance(content, str):
    ...
    for hit in results:
        ...
        if domains:
            ...
            if alignment:
                ...
                query_start = query_end = target_start = target_end = query_length = (
                    target_length
                ) = ""
            ...
            bias = env_from = env_to = query_start = query_end = target_start = (
                target_end
            ) = query_length = target_length = "0"
            identity_pct = identity_count = similarity_pct = similarity_count = "0"
...
def fetch_sequence_chunk(db_path: str, target_names: List[str]) -> dict:
    ...
    try:
        ...
        if not os.path.exists(ssi_path):
            logger.warning(
                f"SSI index not found at {ssi_path}, falling back to individual fetching"
            )
            return {
                name: fetch_sequence_from_database(name, db_path)
...
def fetch_sequences_parallel(
    ...
    logger.info(
        f"Fetching {len(target_names)} sequences in {len(chunks)} chunks from database: {db_path}"
    )
    ...
    try:
        ...
        try:
            ...
            if decompressed == content:
                ...
                logger.warning(
                    "Compression/decompression test failed - content mismatch"
                )
...
def validate_fasta_content(content: str) -> bool:
    ...
    for i, line in enumerate(lines):
        if i % 2 == 0:
            ...
            if line:
                valid_chars = set(
                    "ACGTNacgtnACDEFGHIKLMNPQRSTVWYXacdefghiklmnpqrstvwyx-*"
                )
...
def generate_enhanced_aligned_fasta_content(results, db_path: str, query_input: str):
    ...
    try:
        ...
        if query_match:
            ...
            return generate_fallback_aligned_fasta_content(
        ...
        if not alignment_sequences:
            logger.warning(
                "No valid sequences found for alignment, using fallback method"
            )
            return generate_fallback_aligned_fasta_content(
                results, db_path, query_input
            )
        ...
        for seq_info in alignment_sequences:
            try:
                text_seq = TextSequence(
                    name=seq_info["name"].encode(), sequence=seq_info["sequence"]
                )
                ...
                logger.error(
                    f"Error creating digital sequence for {seq_info['name']}: {seq_error}"
                )
        ...
        try:
            ...
            logger.info(
                f"Compressed aligned FASTA size: {len(compressed_content)} bytes"
            )
            return compressed_content
        except Exception as e:
...
def generate_pyhmmer_msa_content(results, db_path: str, query_input: str):
    ...
    try:
        ...
        for hit in results:
            ...
            if target in sequences and sequences[target] and domains:
                ...
                if seq_match:
                    ...
                    extended_from = max(
                        1, (target_from if alignment else env_from) - 50
                    )
                    extended_to = min(
                    ...
                    logger.debug(
                        f"Extended sequence preview: {extended_sequence[:100]}..."
                    )
                    ...
                    logger.debug(
                        f"Full sequence for {target}: {full_sequence[:100]}..."
                    )
                    ...
                    if domain_sequence:
                        ...
                        if len(domain_sequence) <= 50 and len(alignment_sequences) > 0:
                            ...
                            if alignment_sequences[0]["sequence"] == domain_sequence:
                                logger.info(
                                    f"Sequence {target} is identical to first sequence, using extended region"
                                )
                                alignment_sequences.append(
                                    {
        ...
        for seq_info in alignment_sequences:
            logger.info(
                f"Sequence: {seq_info['name']}, Length: {len(seq_info['sequence'])}"
            )
        ...
        if len(alignment_sequences) > 1:
            ...
            all_identical = all(
                seq["sequence"] == first_seq for seq in alignment_sequences[1:]
            )
...
def generate_fallback_aligned_fasta_content(results, db_path: str, query_input: str):
    ...
    try:
        compressed_content = gzip.compress(content.encode("utf-8"))
        logger.info(
            f"Compressed fallback aligned FASTA size: {len(compressed_content)} bytes"
        )
        return compressed_content
    except Exception as e:
        ...
        logger.warning(
            "Returning uncompressed fallback aligned FASTA content due to compression error"
        )
        return content.encode("utf-8")
...
def get_result(request, id: uuid.UUID, query: Query[ResultQuerySchema]):
    ...
    try:
        ...
        logger.info("Job is successful, getting database info...")
...
def get_domains_by_target(request, id: uuid.UUID, target: str):
    ...
    try:
        logger.info("=== GET DOMAINS BY TARGET ===")
        logger.info(f"Job ID: {id}")
        ...
        if job.task.status != "SUCCESS":
            ...
            raise HttpError(
                400, f"Job not completed successfully, status: {job.task.status}"
            )
        ...
        try:
            if isinstance(job.task.result, str):
                ...
                result_data = json.loads(job.task.result)
                logger.info(
                    f"Successfully parsed JSON result, type: {type(result_data)}"
                )
            else:
            ...
            logger.error(
                f"Task result length: {len(job.task.result) if isinstance(job.task.result, str) else 'N/A'}"
            )

            # Try to recover from truncated JSON
            if isinstance(job.task.result, str) and len(job.task.result) > 1000:
                logger.warning(
                    "Attempting to recover from potentially truncated JSON..."
                )
                try:
                    ...
                    logger.error(
                        f"Failed to recover from truncated JSON: {recovery_error}"
                    )
                    raise HttpError(
                        500,
                        "Job result is corrupted and cannot be recovered. Please try the search again.",
                    )
        ...
        if not target_data:
            ...
            logger.info(
                f"Available targets: {[item.get('target') for item in result_data[:5]]}"
            )
