...
def run_integration_tests(verbose=False, coverage=False):
    ...
    if os.path.exists(test_path):
        success, output = run_pytest_tests(
            test_path, "PyHMMER Integration Tests", verbose, coverage
        )
...
def run_unit_tests(verbose=False, coverage=False):
    ...
    if os.path.exists(test_path):
        success, output = run_pytest_tests(
            test_path, "PyHMMER Unit Tests", verbose, coverage
        )
...
def run_search_tests(verbose=False, coverage=False):
    ...
    if os.path.exists(test_path):
        success, output = run_pytest_tests(
            test_path, "PyHMMER Search API Tests", verbose, coverage
        )
...
def run_results_tests(verbose=False, coverage=False):
    ...
    if os.path.exists(test_path):
        success, output = run_pytest_tests(
            test_path, "PyHMMER Results API Tests", verbose, coverage
        )
...
def run_workflow_tests(verbose=False, coverage=False):
    ...
    if os.path.exists(test_path):
        success, output = run_pytest_tests(
            test_path, "PyHMMER Workflow Tests", verbose, coverage
        )
...
Examples:
    ...
    parser.add_argument(
        "--results", action="store_true", help="Run results API tests only"
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.all:
            ...
            if args.search:
                results.append(
                    ("Search API Tests", run_search_tests(args.verbose, args.coverage))
                )
            ...
            if args.results:
                results.append(
                    (
                        "Results API Tests",
                        run_results_tests(args.verbose, args.coverage),
                    )
                )

            if args.workflow:
                results.append(
                    ("Workflow Tests", run_workflow_tests(args.verbose, args.coverage))
                )
