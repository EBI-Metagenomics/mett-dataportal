        ...
        with psycopg.connect(
            ...
            with conn.cursor() as cursor:
                ...
                for (
                    ...
                    cursor.execute(
                        "SELECT id FROM Gene WHERE locus_tag = %s", (gene_id_tuple,)
                    )
                ...
                logging.info(
                    f"Updated Strain table with {os.path.basename(gff_file)} for isolate: {isolate}"
                )
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for gff_file in gff_files_in_isolate:
                    if gff_file.endswith("_annotations.gff"):
                        futures.append(
                            executor.submit(
                                process_gff_file, gff_file, isolate, strain_id
                            )
                        )
...
if __name__ == "__main__":
    try:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [
                executor.submit(process_isolate, isolate) for isolate in isolates
            ]
            for future in futures:
                future.result()
