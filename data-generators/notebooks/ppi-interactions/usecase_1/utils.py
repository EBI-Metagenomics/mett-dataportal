...
def load_gff_annotation(gff_path):
    ...
    gff_df["gene_id"] = gff_df["attributes"].apply(
        lambda x: re.search(r"ID=(\S+?);", x).group(1)
    )
    gff_df["locus_tag"] = gff_df["attributes"].apply(
        lambda x: (
            re.search(r"locus_tag=(\S+?);", x).group(1)
            if re.search(r"locus_tag=(\S+?);", x)
            else None
        )
    )
    gff_df["uniprot_id"] = gff_df["attributes"].apply(
        lambda x: (
            re.search(r"UniProt:(\S+?);", x).group(1)
            if re.search(r"UniProt:(\S+?);", x)
            else None
        )
    )
    gff_df["name"] = gff_df["attributes"].apply(
        lambda x: (
            re.search(r"product=([^;]*);", x).group(1)
            if re.search(r"product=([^;]*);", x)
            else None
        )
    )
...
def plot_network(G):
    ...
    nx.draw_networkx_edges(
        G, pos=pos, ax=ax, width=[d["weight"] * 2 for (_, _, d) in G.edges(data=True)]
    )
